...
export class LoginComponent implements OnInit {
  ...
  public ngOnInit(): void {
    ...
    this.ordersService.getAllOrders().subscribe(o => this.allOrders = o);
      ...
      this.allUsers.map(u => {
        if (u.email === this.email && u.password === this.password) {
          ...
          this.cartsService.getCartFromUser(u._id).subscribe(cart => {
            ...
            if (this.cart && this.cart[0]) {
              this.alertStyle = "info";
              this.username = "Welcome " + u.private_name;
              this.message = `Welcome back to our site, to continue your shopping from ${this.cart[0].date} click the button below.`;
              this.startShopping = undefined;
